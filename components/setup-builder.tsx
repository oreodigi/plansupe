...
export function SetupBuilder({
  ...
  const catalog = useMemo(
    () =>
      basics.category
        ? MODULES.flatMap(({ key }) => requirementsFor(basics.category, key))
        : [],
    [basics.category],
  );
  const selectedItems = catalog.filter((entry) =>
    requirements.includes(entry.id),
  );
  ...
  function updateBasic(key: keyof Basics, value: string) {
    ...
    if (key === "category" && value !== basics.category) {
      setModules([]);
      setRequirements([]);
  ...
  function toggleModule(module: ModuleKey) {
    ...
    if (selected)
      setRequirements((current) =>
        current.filter(
          (id) =>
            !requirementsFor(basics.category, module).some(
              (entry) => entry.id === id,
            ),
        ),
      );
  ...
  function next() {
    ...
    if (
      ...
      setClientError(
        "Complete the business name, category, city, budget and target date to continue.",
      );
      return;
    }
    if (step === 1 && modules.length === 0) {
      setClientError("Choose at least one module for this business.");
      return;
    }
    if (step === 2 && requirements.length === 0) {
      setClientError(
        "Choose at least one requirement to build your setup plan.",
      );
      return;
    }
  ...
  return (
    <form className="setup-builder" action={formAction}>
      <input type="hidden" name="name" value={basics.name} />
      <input type="hidden" name="category" value={basics.category} />
      <input type="hidden" name="stage" value={basics.stage} />
      <input type="hidden" name="city" value={basics.city} />
      <input type="hidden" name="budget" value={basics.budget} />
      <input type="hidden" name="launchDate" value={basics.launchDate} />
      ...
      <header className="setup-progress">
        <div className="setup-progress-copy">
          <span>
            {editing ? "Configure workspace" : `Welcome, ${firstName}`}
          </span>
          <strong>
            {
              [
      ...
      <main className="setup-stage">
        ...
        {step === 3 && (
          <ReviewStep basics={basics} modules={modules} items={selectedItems} />
        )}
      </main>

      <footer className="setup-footer">
        <div className="selection-tally">
          <span>{requirements.length} requirements</span>
          <b>{money(estimated)}</b>
          {budget > 0 && (
            <small className={estimated > budget ? "over" : ""}>
              {estimated > budget
                ? `${money(estimated - budget)} over budget`
                : `${money(budget - estimated)} budget remaining`}
            </small>
          )}
        </div>
...
function BasicsStep({
  ...
  basics: Basics;
  update: (key: keyof Basics, value: string) => void;
}) {
  return (
    <section className="setup-panel">
      <div className="setup-intro">
        ...
        <div>
          <p className="kicker">Start with the essentials</p>
          <h1>Tell us what you’re building.</h1>
          <p>
            These details help PlanSupe tailor the setup options, costs and
            timeline to your business.
          </p>
        </div>
      </div>
      <div className="wizard-fields">
        ...
        <label className="field wide">
          ...
          <div className="input-prefix">
            ...
            <input
              type="date"
              value={basics.launchDate}
              onChange={(event) => update("launchDate", event.target.value)}
            />
          </div>
          <small>
            We’ll work backwards from this date when scheduling selected
            requirements.
          </small>
        </label>
      </div>
...
function ModulesStep({
  ...
  selected: ModuleKey[];
  toggle: (module: ModuleKey) => void;
}) {
  return (
    <section className="setup-panel">
      <div className="setup-intro compact">
        <div>
          <p className="kicker">Your workspace, your choice</p>
          <h1>Which areas do you want to plan?</h1>
          <p>
            Choose only the modules that matter now. No requirements are added
            until you select them next.
          </p>
        </div>
...
function RequirementsStep({
  ...
  return (
    <section className="setup-panel requirements-panel">
      <div className="setup-intro compact">
        <div>
          <p className="kicker">Tailored for {category}</p>
          <h1>Choose what you actually need.</h1>
          <p>
            Licence requirements explain when they apply and which authority to
            check. Rules can vary by state and city.
          </p>
        </div>
      </div>
      <div className="requirement-layout">
        <nav className="module-rail" aria-label="Selected modules">
          {modules.map((module) => {
            const Icon = moduleIcons[module];
            const count = selected.filter((id) =>
              requirementsFor(category, module).some(
                (entry) => entry.id === id,
              ),
            ).length;
            return (
              <button
                type="button"
                key={module}
                className={current === module ? "active" : ""}
        ...
        <div className="requirement-list">
          {options.map((option) => {
            ...
            return (
              <button
                type="button"
                key={option.id}
                className={`requirement-option ${checked ? "selected" : ""}`}
              ...
              >
                ...
                <span className="requirement-copy">
                  ...
                  {option.authority && (
                    <span className="compliance-context">
                      <em>
                  ...
                  <span className="requirement-meta">
                    <em>
                      {option.estimatedCost
                        ? money(option.estimatedCost)
                        : "Fee varies by authority"}
                    </em>
                    <em>
                      Start {option.leadWeeks}{" "}
                      {option.leadWeeks === 1 ? "week" : "weeks"} before launch
                    </em>
                  </span>
                  {option.sourceUrl && (
          ...
          <div className="custom-note">
            ...
            <div>
              ...
              <span>
                You can add custom requirements from the setup plan after
...
function ReviewStep({
  ...
  return (
    <section className="setup-panel">
      <div className="setup-intro">
        <span className="step-art success">
          <CheckCircle weight="duotone" />
        </span>
        <div>
          <p className="kicker">Ready to build</p>
          <h1>Your workspace reflects your choices.</h1>
          <p>
            Review the plan below. You can reconfigure modules and add custom
            requirements later.
          </p>
        </div>
      </div>
      <div className="review-summary">
        ...
        <div>
          ...
          <b>
            {basics.launchDate
              ? new Date(`${basics.launchDate}T12:00:00`).toLocaleDateString(
                  "en-IN",
                  { day: "numeric", month: "short", year: "numeric" },
                )
              : "Not set"}
          </b>
          <small>{money(Number(basics.budget || 0))} budget</small>
        </div>
      </div>
      <div className="review-modules">
        {modules.map((module) => {
          const Icon = moduleIcons[module];
          const count = items.filter((entry) => entry.module === module).length;
          return (
            <div key={module}>
              <Icon weight="duotone" />
              <span>
